...
- [x] **5-1. 동행 완료 처리 CTA (`DashboardModal.tsx`, `ChatView.tsx`)** `[P0]`
  - [동행이 잘 끝났어요 (블라인드 평가 & 당도 정산)] 및 채팅방 상단 [만남 완료 & 평가 ⭐] 버튼 완비
- [x] **5-2. 상호 블라인드 평가 모달 (`ReviewModal.tsx`)** `[P0]`
  - 평가 항목: 별점(1~5점), 칭찬 뱃지 6종 다중선택, 한줄 정성 피드백
  - **블라인드 원칙**: 내가 평가를 제출해도 상대방이 제출하기 전까지는 상대 점수 비공개(잠금 대기 🔒)
- [x] **5-3. 동시 해제 및 실시간 당도(Sugar Content) 정산** `[P0]`
  - 양측 모두 평가 완료 시 평가 결과 동시 공개(✨)
  - 긍정 평가 시: 정수형 **당도 즉시 상승** (예: `+2 🍯` 가산 및 최대 100 고정 정수 연산)
  - 노쇼/비매너 신고 접수 시: 당도 차감 및 경고 조치
- [x] **5-4. 마이페이지 후기 탭 및 당도 히스토리 (`MyPageView.tsx`)** `[P1]`
  - 프로필 서브탭: [동행 정보 & 설정] / [받은 칭찬 & 후기 ✨]
  - 칭찬 뱃지 4종 컬렉션 카드 및 블라인드 해제된 동행러 후기 피드 노출


---

### [Phase 6 - 확장] 유료 전문 동행 및 오퍼 관리 (PAID OFFER)
> **목표**: 전문 지식(스냅 촬영, 러닝 코칭, 외국어 가이드 등)을 제공하는 1:1 유료 동행 및 안심 에스크로 결제 확장

- [x] **6-1. 유료 오퍼 등록 폼 (`CreateMeetupModal.tsx`)** `[P2]`
  - 일반 동행(무료) vs PRO 전문 동행(유료) 선택 토글
  - 시간당 비용, 전문 분야, 포함/불포함 내역, 활동 커리큘럼 명시
- [x] **6-2. 견적 제안 및 1:1 안심 에스크로 결제 플로우 (`EscrowPaymentModal.tsx`, `PostDetailModal.tsx`)** `[P2]`
  - 동행 시간(1h/2h/3h) 선택에 따른 총 금액 실시간 산출
  - 유미당 안심 에스크로 100% 보증(동행 완료 및 확인 시까지 대금 안전 예치)
  - 원클릭 간편결제(카카오페이, 토스페이, 안심카드) 및 즉시 1:1 확정(2/2명 잠금) 연동
  - 마이페이지 내 에스크로 결제/예치 내역 및 PRO 파트너 등록 탭 제공 (`MyPageView.tsx`)
