...
- 마지막 run: `run-codex-preflight1`
- 재개 지점: 모든 단계 pass — 회귀는 `npm run test:harness`

| 단계 | 상태 | run_id | 단계 결과 | 마지막 통과 단계 | 실패 원인 | 재실행 명령 |
|---|---|---|---|---|---|---|
| 사전 검사 | `pass` | run-codex-preflight1 | PASS 7 | local migrations are named with remote versions | - | `npm run harness:preflight` |
| 기능 1 회원가입 | `pass` | run-ui-api1b | PASS 9 | logout revokes the refresh token | - | `npm run harness:01` |
| 기능 2 로그인 | `pass` | run-ui-api1b | PASS 6 | refresh keeps the user; logout revokes only that session | - | `npm run harness:02` |
