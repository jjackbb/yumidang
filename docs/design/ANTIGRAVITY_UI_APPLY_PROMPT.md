...
예: `S01 둘러보기`, `docs/design/stitch-export/S01_둘러보기.png` (+ HTML이 있으면 함께)
...
화면 요구사항: docs/design/STITCH_PROMPTS.md 의 {화면 ID} 항목

## 반드시 지킬 원칙
1. 동작 로직은 바꾸지 않는다.
   - props, state, 이벤트 핸들러, API 호출(src/live/*), 유틸(src/utils/*), 타입(src/types.ts), 검증 규칙을 수정하지 않는다.
...
## 순서
1. 대상 컴포넌트 파일과 이를 여는 src/App.tsx 부분을 읽고, 화면에 있는 모든 상태·분기를 목록으로 정리한다.
...
4. 다음을 실행하고 모두 통과해야 완료다:
   ...
   - npm run build
   - git diff --check
5. npm run dev 로 http://localhost:3000 에서 모바일 폭(390px)으로 해당 화면을 열어 스크린샷을 남긴다(비로그인에서 볼 수 있는 상태 위주). 경로: docs/design/applied/{화면 ID}.png
...
| ID | 화면 | 주요 파일 |
|---|---|---|
| S01 | 둘러보기 | `src/components/ExploreView.tsx` |
| S02 | 공고 상세 | `src/components/PostDetailModal.tsx` |
| S03·S04 | 공고 작성 1·2단계 | `src/components/CreateMeetupModal.tsx` |
| S05~S07 | 로그인·가입 | `src/components/AuthModal.tsx`, `PhoneInput.tsx` |
| S08 | 참여 신청 | `src/components/JoinRequestModal.tsx`, `LifecycleDialogs.tsx`(일정 충돌만) |
