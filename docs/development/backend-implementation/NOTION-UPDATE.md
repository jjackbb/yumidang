...
이 문서는 실행 프롬프트가 지정한 Notion 페이지에 직접 기록할 수 없을 때 사용하는 로컬 대체 기록이다. 기능별 상세 키·제약·RLS/RPC 계약은 `docs/development/backend-plans/02-login-HANDOFF.md`부터 `09-mutual-review-HANDOFF.md`까지, 실제 실행 증거는 `docs/development/backend-implementation/evidence/`에 있다.
