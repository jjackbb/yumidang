...
실패하면 첫 실패 단계에서 멈추고 `docs/development/backend-implementation/STATE.md`에 마지막 통과 단계·실패 원인·재실행 명령이 남는다.
...
- `docs/development/backend-implementation/evidence/<run_id>/<단계>.json` — 단계별 PASS/FAIL/NOT_RUN, 계층(LOCAL/REMOTE/BROWSER), 소요 시간, 안전한 수치(지연 시간 등)
