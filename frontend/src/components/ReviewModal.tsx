...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, appointment, partnerName, variant, state, ownReview, partnerReview, onSubmitReview, ratingAndCommentOnly = false }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const result = await onSubmitReview({ rating, positiveItems, negativeItems, comment });
  ...
  return <div role="dialog" aria-modal="true" aria-label="동행 평가" className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4">
    <div className="bg-white w-full max-w-[440px] rounded-t-[28px] sm:rounded-[28px] max-h-[92vh] overflow-y-auto text-left shadow-2xl">
      ...
      {state.reviewsReleased && partnerReview ? <div className="p-5 space-y-4">
        <div className="rounded-2xl bg-emerald-50 p-4 text-center"><Unlock className="mx-auto text-emerald-600" /><h3 className="mt-2 font-bold">후기가 공개됐어요</h3><p className="mt-1 text-xs text-gray-600">{state.releaseReason === 'deadline' ? '평가 기한이 끝나 제출된 후기가 공개됐어요.' : '양쪽 평가가 도착해 공개 보류 종료 뒤 함께 공개됐어요.'}</p></div>
        <ReviewCard review={partnerReview} partnerName={partnerName} />
        ...
        <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${state.reviewsReleased ? 'bg-emerald-50 text-emerald-600' : 'bg-purple-50 text-[#6c2cf5]'}`}>{state.reviewsReleased ? <Unlock size={28} /> : <Lock size={28} />}</div>
        <div><h3 className="font-bold">{state.reviewsReleased ? '내 후기가 공개됐어요' : '내 평가를 제출했어요'}</h3><p className="mt-2 text-xs leading-relaxed text-gray-500">{state.reason}</p></div>
        <div className="rounded-2xl bg-gray-50 p-4 text-left text-xs"><p className="flex items-center justify-between"><span>내 평가</span><b className="text-emerald-600">제출 완료</b></p><p className="mt-2 flex items-center justify-between"><span>상대 평가</span><b className={state.hasOtherReview ? 'text-emerald-600' : 'text-amber-600'}>{state.hasOtherReview ? '제출 완료' : '제출 대기'}</b></p></div>
        ...
        <div className="rounded-2xl bg-purple-50 p-3 text-xs leading-relaxed text-purple-950"><b className="flex items-center gap-1"><Lock size={14} />블라인드 평가</b><p className="mt-1">완료 알림 뒤 24시간은 공개가 보류돼요. 양쪽 평가가 있으면 보류 종료 즉시, 한쪽만 있으면 7일 기한에 공개됩니다.</p>{deadline && <p className="mt-1 text-purple-700">작성 기한: {deadline} 직전까지</p>}</div>
