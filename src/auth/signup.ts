...
export function signupErrorMessage(error: SupabaseLikeError, action: 'send' | 'verify' | 'profile') {
  const haystack = `${error.code || ''} ${error.message || ''}`.toLowerCase();
  if (error.status === 429 || haystack.includes('rate limit') || haystack.includes('over_sms_send_rate_limit'))
  ...
  if (action === 'send') return '인증번호 요청에 실패했어요. 번호와 네트워크 상태를 확인해 주세요.';
