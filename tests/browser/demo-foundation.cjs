...
(async () => {
  ...
  async function run(id, name, fn, viewport = { width: 390, height: 844 }) {
    ...
    page.on('request', request => {
      if (/google-analytics|googletagmanager|generativelanguage/.test(request.url())) result.externalRequests.push(`${id}: ${request.url()}`);
    });
    try {
  ...
  await run('S5', 'demo sends no external analytics traffic across auth, post detail and logout', async page => {
