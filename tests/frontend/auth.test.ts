...
import { currentUserFromProfile } from '../../frontend/src/auth/user.ts';
import {
  authErrorMessage,
  exactAgeLabel,
  toE164KoreanPhone,
  ...
  type SignupProfile,
} from '../../frontend/src/auth/signup.ts';
import {
  SupabaseConfigError,
...
test('real-name masking matches the server rule', async () => {
  const { maskRealName } = await import('../../frontend/src/utils/maskName.ts');
...
test('login return path accepts only internal tab paths', async () => {
  const { safeReturnPath, loginPath } = await import('../../frontend/src/auth/routes.ts');
