...
import { toLiveError } from '../../frontend/src/live/errors.ts';
...
test('existing-screen adapters map server rows without inventing sample data', async () => {
  const { toMeetupPost, toJoinRequest, toAppointmentReviews, toNotification, REQUEST_STATUS_TO_SCREEN } = await import('../../frontend/src/live/adapters.ts');
...
test('service-mode post form accepts only 시·구·동 public areas and requires the private place', async () => {
  const { defaultPostForm, validatePostForm } = await import('../../frontend/src/utils/postForm.ts');
