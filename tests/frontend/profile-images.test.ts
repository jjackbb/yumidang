...
import { runPhotoSignup, PhotoSignupFlowError } from '../../frontend/src/auth/signupPhotoFlow.ts';
import {
  createProfileImagePath, isDirectlyRenderableAvatar, isOwnProfileImagePath, isProfileImagePath,
  PROFILE_IMAGE_MAX_STORED_BYTES,
} from '../../frontend/src/profile/avatarPath.ts';
import {
  clearMyProfileImage,
...
} from '../../frontend/src/profile/avatarStorage.ts';
import { containedImageSize, PROFILE_IMAGE_JPEG_QUALITY, PROFILE_IMAGE_MAX_EDGE } from '../../frontend/src/utils/imageResize.ts';
import { avatarSrc, PLACEHOLDER_AVATAR, profileStepsForMode } from '../../frontend/src/utils/profile.ts';
import type { CurrentUser } from '../../frontend/src/types.ts';
...
test('expansion migration defines private bucket, owner policies, validating versioned RPCs, and transitional old-RPC safety', () => {
  const sql = readFileSync(new URL('../../backend/supabase/migrations/20260917052827_profile_images_required.sql', import.meta.url), 'utf8');
...
test('contraction revokes only authenticated legacy signup execution', () => {
  const sql = readFileSync(new URL('../../backend/supabase/migrations/20260917094753_disable_legacy_signup_without_avatar.sql', import.meta.url), 'utf8');
...
test('production and reusable harness signup paths no longer call the legacy RPC', () => {
  const production = readFileSync(new URL('../../frontend/src/auth/signupEligibility.ts', import.meta.url), 'utf8');
...
test('storage adapter requests JPEG upload without upsert and centralizes signed URL creation', () => {
  const source = readFileSync(new URL('../../frontend/src/profile/avatarStorage.ts', import.meta.url), 'utf8');
