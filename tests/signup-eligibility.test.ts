...
} from '../src/auth/signupEligibilityValidation.ts';
import {
  createTestEmailAuthHandler,
  PROJECT_URL,
  type TestEmailAuthConfig,
} from '../supabase/functions/test-institutional-email-auth/handler.ts';
...
test('migration contract locks private data and removes direct profile insert', () => {
  const sql = readFileSync(new URL('../supabase/migrations/20260917043418_signup_eligibility_author_demographics.sql', import.meta.url), 'utf8');
